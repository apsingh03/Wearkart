...
const createParentActressCarousel = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const { name, animation, timer, width, height, objectFit } = req.body;
    const nameAlreadyExist = await ActressCarousel.findOne({
    ...
    if (nameAlreadyExist) {
      ...
      const createQuery = await ActressCarousel.create(
        {
          name,
          animation,
          timer,
      ...
      const updatedQuery = await ActressCarousel.findOne({
...
const getParentActressCarousel = async (req, res) => {
  try {
...
const updateParentActressCarousel = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    // console.log("req.body - ", req.body.updatedData);

    const [updated] = await ActressCarousel.update(
      {
    ...
    if (updated) {
      ...
      const query = await ActressCarousel.findOne({
        where: { id: req.params.id, admin_id: req.admin.id },
        transaction: t,
      });
      await t.commit();
...
const deleteParentActressCarousel = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const query = await ActressCarousel.destroy({
      where: { id: req.params.id, admin_id: req.admin.id },
      transaction: t,
    });
...
const actressCarouselIsFavorite = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    ...
    if (isFavoriteStatus === "favoriteIt") {
      // want to favorite it
      const query = await ActressCarousel.update(
        {
          isFavorite: 1,
        },
...
module.exports = {
  createParentActressCarousel,
  getParentActressCarousel,
  updateParentActressCarousel,
  deleteParentActressCarousel,
  actressCarouselIsFavorite,
};
