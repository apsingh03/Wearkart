...
import { getParentActressCarouselAsync } from "../../../Redux/AdminSlices/ActressCarousel/parentActressCarouselSlice";
import {
  childActressCarouselIsFavoriteAsync,
  createChildActressCarouselAsync,
  deleteChildActressCarouselAsync,
  getChildActressCarouselAsync,
  updateChildActressCarouselAsync,
} from "../../../Redux/AdminSlices/ActressCarousel/childActressCarouselSlice";

const ChildActressCarousel = () => {
  const admin_parentActressCarouselRedux = useSelector(
    (state) => state.admin_parentActressCarousel.data
  );

  const admin_childActressCarouselRedux = useSelector(
    (state) => state.admin_childActressCarousel.data
  );
  ...
  const validationSchema = Yup.object().shape({
    cardName: Yup.string().min(2, "Too Short!").required("*Required"),
  ...
  async function deleteHandler(event, id) {
    ...
    if (window.confirm("Are you sure want to Delete It ?")) {
      const actionResult = await dispatch(
        deleteChildActressCarouselAsync({ id })
      );
  ...
  async function fetchData() {
    setisLoadingTopProgress(30);
    await dispatch(getParentActressCarouselAsync());
    await dispatch(getChildActressCarouselAsync());
                      ...
                      <div className="form-group mb-3">
                        ...
                        >
                          ...
                          {(function () {
                            try {
                              return (
                                admin_parentActressCarouselRedux.query &&
                                admin_parentActressCarouselRedux.query.map(
                                  (data, idx) => {
                                    return (
            ...
            <table className="table">
              ...
              <tbody>
                {(function () {
                  try {
                    return (
                      admin_childActressCarouselRedux.query &&
                      admin_childActressCarouselRedux.query.map(
                        (data, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td>
                                {
                                  data?.actressCarouselImagesActressCarousel
                                    ?.name
                                }
                              </td>
...
export default ChildActressCarousel;
